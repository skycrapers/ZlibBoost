"""Hidden power simulation executor."""
...
class HiddenSimulationExecutor(BaseSimulationExecutor):
    """Executor for hidden power characterization decks."""

    def __init__(self, parser_registry: MeasurementParserRegistry | None = None, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self._parser_registry = parser_registry or MeasurementParserRegistry(
            [HiddenSpectreParser(), HiddenHspiceParser()]
        )
    ...
    def _resolve_engine_workdir(self, deck_path: Path, metadata: Dict[str, Any], output_dir: Path) -> Path:
        ...
        sim_type = metadata.get("sim_type", "hidden")
    ...
    def _write_artifacts(self, result: Dict[str, Any], output_dir: Path, engine_dir: Path) -> None:
        ...
        artifact_path = sim_dir / f"{arc_id}_hidden.json"
